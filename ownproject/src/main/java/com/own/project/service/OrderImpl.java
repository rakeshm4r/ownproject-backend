...
public class OrderImpl implements OrderDao {
    ...
    @Autowired
    private OrdersStatusRepo ordersStatusRepo;

    @Autowired
    private PaymentRepo paymentDetailsRepo;

    @Autowired
    private UserTypeDetailsRepo userTypeDetailsRepo;

    @Autowired
    private UserTypeDetailsImpl userTypeDetailsImpl;
    ...
    public void updateOrderStatus(Long ordersStatusId, String orderStatus, String reason, Long modifiedUserId) {
        ...
        if (orderStatus.equalsIgnoreCase("packed&shipped")) {
            invoiceDetails.setOrderStatus(ordersStatusData);
            invoiceDetails.setUser(ordersStatusData.getUser());
            String invoiceNumber = nameUtils.generateInvoiceNumber();
            invoiceDetails.setInvoiceNumber(invoiceNumber);
            log.info("Invoice Details  Is Saved ");
            invoiceDetailsRepo.save(invoiceDetails);

            ordersStatusData.setInvoiceDetails(invoiceDetails);
        }
